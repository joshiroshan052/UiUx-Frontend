...
export default function ViewBox({ stories }) {
  ...
  const { id: storyId } = useParams(); // Use useParams to get the story ID

  useEffect(() => {
    if (!stories || !stories.current) return;

    const { owner } = stories.current;

    if (!owner || !storyId) {
      console.error('Owner or Story ID is undefined');
      return;
    }

    // Fetch the user details and mark the story as seen
    api.get(`${url}/user/get/${owner}`)
      .then((res) => {
        setUser(res.data);
        if (stories.current.owner !== user._id) {
          return api.put(`${url}/story/seen/${storyId}`);
        }
      })
      .then((res) => res && console.log(res.data))
      .catch((err) => console.log(err));

    // Handle progress bar and navigation
